...
const TOC_MIN_SECTIONS = 3;   // 少于三节的短回答不值得加目录
...
function bindEvents() {
  ...
  elements.chatList.addEventListener("click", handleTocClick);
...
function renderAssistantBody(body, message) {
  ...
  if (message.streaming) {
    // 生成中不插目录：章节还在长，插了会一直跳
    body.insertAdjacentHTML("beforeend", '<span class="streaming-cursor" aria-label="正在生成"></span>');
    return;
  }
  injectToc(body, message);
}

/**
 * 目录从**导读正文里实际写出的二级标题**提取，不由模型另外输出一份。
 * 导读的章节结构是跟着论文走的，写死一份目录必然对不上。
 */
export function sectionTitles(markdown) {
  ...
  for (const line of String(markdown || "").split("\n")) {
    ...
    if (inFence) continue;
    const heading = line.match(/^##\s+(.+?)\s*$/);   // 只取二级，### 不进目录
    if (heading) titles.push(heading[1].replace(/[*`]/g, "").trim());
  }
  return titles;
}

function injectToc(body, message) {
  const titles = sectionTitles(message.text);
  if (titles.length < TOC_MIN_SECTIONS) return;   // 追问的短回答不需要目录
  const items = titles
    .map((title, index) => `<li><a href="#" data-section="${index}">${escapeHtml(title)}</a></li>`)
    .join("");
  body.insertAdjacentHTML("afterbegin",
    `<nav class="summary-toc" aria-label="导读目录">` +
    `<p class="summary-toc-caption">目录 · ${titles.length} 节</p>` +
    `<ol>${items}</ol></nav>`);
}

/** 目录用事件委托：正文每次重渲染都会换掉节点，逐个绑监听会漏。 */
function handleTocClick(event) {
  const link = event.target?.closest?.(".summary-toc a[data-section]");
  if (!link) return;
  event.preventDefault();
  const body = link.closest(".message-body");
  const heading = body?.querySelectorAll("h2")[Number(link.dataset.section)];
  if (!heading) return;
  // 点目录就是要离开底部，别再被流式跟随拽回去
  state.stickToBottom = false;
  const listTop = elements.chatList.getBoundingClientRect().top;
  const target = heading.getBoundingClientRect().top - listTop + elements.chatList.scrollTop - 8;
  elements.chatList.scrollTo({ top: Math.max(0, target), behavior: "smooth" });
}
